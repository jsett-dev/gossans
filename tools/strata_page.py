...
function gossansStrata() {
  ...
  function filters() {
    ...
    function checks(name, labels, key) {
      ...
      return '<details><summary>' + name + '</summary>' + labels.map(function (l, i) {
        return c[i] ? '<label><input type="checkbox" data-f="' + key + '" value="' + i + '" checked> ' +
               l + '<span class="n">' + c[i].toLocaleString() + '</span></label>' : "";
      }).join("") + "</details>";
    }
    var states = {};
  ...
  function wellCard(wi, pad) {
    ...
    card.innerHTML =
      ...
      '<span class="m gnote">Ground here is the ' + Math.round(terr ? terr.step_m : 0) + " m grid until the one-metre model answers for this window.</span>" +
    ...
    fetch("/data/contours.json").then(function (r) { return r.ok ? r.json() : null; })
      .then(function (doc) {
        ...
        doc.features.forEach(function (f) {
          var c = f.geometry.coordinates, z = f.properties.z;
          var shade = f.properties.index ? 0.30 : 0.62;
          for (var i = 1; i < c.length; i++) {
            pts.push((c[i - 1][0] - data.origin.lon) * m.lon, (c[i - 1][1] - data.origin.lat) * m.lat, z + 3,
                     (c[i][0] - data.origin.lon) * m.lon, (c[i][1] - data.origin.lat) * m.lat, z + 3);
  ...
  function buildPadGround(wi) {
    ...
    fetch(url).then(function (r) { return r.ok ? r.arrayBuffer() : null; }).then(function (buf) {
  ...
  function padUv(lon0, lat0, stepLon, stepLat, nx, ny) {
    var z = 14, n = 1 << z;
  ...
  function applyGround() {
    ...
    m.needsUpdate = true;
    if (isolated >= 0) buildPatch(isolated);
  }
  ...
  function applyVisible() {
    ...
    if (contourLines) contourLines.visible = showContours && isolated < 0;
